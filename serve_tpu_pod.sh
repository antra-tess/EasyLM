...
gcloud compute tpus tpu-vm ssh finetune-70b --zone=us-central2-b --worker=all --command="export HF_TOKEN='${HF_TOKEN}' && cd ~ && (if [ -d 'EasyLM' ]; then cd EasyLM && git pull; else git clone https://github.com/antra-tess/EasyLM.git && cd EasyLM; fi) && chmod +x ~/EasyLM/worker_serve.sh && ./worker_serve.sh"
